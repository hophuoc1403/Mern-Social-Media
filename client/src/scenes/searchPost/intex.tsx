      ...
      <TextField
        ...
        InputProps={{
          endAdornment: <Box sx={{width: "30px"}}>{isSearching ? <CircularProgress size={20}/> :
            <SearchOutlined fontSize={"small"}/>}</Box>
        }}
        sx={{mb: 2}}
        fullWidth
        autoFocus
      />
      {debouncedValue === "" ? <Typography>Type something to search</Typography> : <Typography>Search results for <strong>{debouncedValue}</strong></Typography>}
      {posts.length > 0 ? posts.map((post: IPost) => (
        <PostWidget key={post.id} {...post} />
      )) : <Typography textAlign={"center"} mt={3} variant={"h3"}></Typography>}
    </Box>
